...
class Entry{
    constructor(owner,car,licensePlate,entryDate,exitDate,entryTime,exitTime){
        this.owner = owner;
        this.car = car;
...
class UI{
    ...
    static addEntryToTable(entry){
        const tableBody = document.querySelector('#tableBody');
        const existingEntries = tableBody.querySelectorAll('.license-plate');

        const isEntryPresent = Array.from(existingEntries).some(cell =>
         cell.textContent === entry.licensePlate);

    if (isEntryPresent) {
    UI.showAlert('Entry with the same License Plate already exists!', 'danger');
    return;
  }
    ...
    static validateInputs(){
        ...
        if(owner === '' || car === '' || licensePlate === '' || entryDate === '' || exitDate === '' || entryTime === '' || exitTime === ''){
...
class Store{
    ...
    static addEntries(entry){
        const entries = Store.getEntries();
        //Check if entry already exists in memory
        const isEntryPresent = entries.some((existingEntry) => existingEntry.licensePlate === entry.licensePlate);

        if (isEntryPresent) {
            console.error("Entry already exists in memory")
        }else{
            entries.push(entry);
...
//Event Add
    document.querySelector('#entryForm').addEventListener('submit',(e)=>{
        ...
        //Instatiate Entry
        const entry = new Entry(owner, car, licensePlate, entryDate, exitDate, entryTime, exitTime);
        //Add the entry do the UI table
